...
class OrderController extends ClientController
{
    ...
    public function manager(Request $request)
    {
        ...
        if(!$user && !$customer){
            // return redirect()->route('client.account.login', ['next' => URL::current()]);
    ...
    public function placeOrder(Request $request)
    {
        ...
        elseif($payment->method == 'cod'){
            ...
            session($session);
            return redirect()->route('client.payments.'.$payment->method)->with('disableBack', 1);
        }
        $request->session()->put('is_empty_cart', 1);
        if($this->cartRepository->useCookie){
            
            return redirect()->route('client.alert')->with($alert)->withCookie(cookie('cart_id', null, -1))->with('disableBack', 1);
        }
        return redirect()->route('client.alert')->with($alert)->with('disableBack', 1);
