...
class ThemeRepository extends BaseRepository
{
     ...
     */
    protected $webDataRepository = null;
    ...
    public function activeSearchMode()
    {
        ...
        return $this->where(function ($query) {
            $query->whereIn('privacy', ['published', 'publish', 'public', 'protected']);//->orWhere('owner_id', $this->getOwnerID());
        })->where('available', 1);
    }
    ...
    public function activePublishMode()
    {
        // $this->resetDefaultParams('owner');
        return $this->where(function ($query) {
            $query->whereIn('privacy', ['public', 'publish', 'published']);
        })->where('available', 1);
    }
    ...
    public function createMetaData($id)
    {
        ...
        $rs = $componentRepository->createListData('blade', 'theme', $theme->id, $components, true);
        $filemanager->setDir($config . '/options');
