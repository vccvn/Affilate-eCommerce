...
class Html
{
    ...
    public function __construct(HtmlAreaList $htmlAreaList)
    {
        $this->htmlAreaList = $htmlAreaList;
    }
    ...
    {
        return $this->htmlAreaList->get($slug);
    }
    ...
    public function addToHead($tag = 'link', $attributes = [])
    {
        $this->heads[] = compact('tag', 'attributes');
    }

    public function getHeadElements()
    {
        $a = '';
        foreach ($this->heads as $key => $data) {
            $tag = new HtmlDom($data['tag'], null, $data['attributes']);
            $a .= $tag->render();
        }
        return $a;
    }
