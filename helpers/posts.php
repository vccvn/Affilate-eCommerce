...
if (!function_exists('get_parse_query_args')) {
    ...
    function get_parse_query_args(array $params = [])
    {
        return array_merge([
            'trashed_status' => 0,
            'privacy' => ['public', 'publish', 'published'],
            'approved' => 1
        ], $params);
    }
}
...
if (!function_exists('get_post_categories')) {
    ...
    function get_post_categories(array $params = [])
    {
        ...
        $rep = app(CategoryRepository::class)->mode('mask')->avaliableCategory();
        
        if (count($categories = $rep->getCategories(get_parse_query_args($params)))) {
            return $categories;
        }
        return [];
    }
...
if (!function_exists('get_post_category_map')) {
    /**
     * lấy map danh mục thuộc tinh
     * @param mixed $args
     * @return array
     */
    function get_post_category_map($args = [])
    {
...
if (!function_exists('get_page')) {
    ...
    {
        // dd(get_parse_query_args($params));
        // $params['a.a'] = 0;
...
if (!function_exists('get_secret_page')) {
    /**
     * lấy mục nội dung
     * @param array $params
     * @return \App\Models\Page
     */
    function get_secret_page(array $params = [])
    {
        // dd(get_parse_query_args($params));
        // $params['a.a'] = 0;
        return app(PageRepository::class)->mode('mask')->detail($params);
    }
}
...
if (!function_exists('get_tags')) {
    ...
    {
        return app(TagRepository::class)->getTags($args);
    }
}
...
if (!function_exists('get_ref_tags')) {
    ...
    {
        return app(TagRepository::class)->getRefTagOptions($ref, $ref_id, $args);
    }
}
...
if (!function_exists('get_tag_data')) {
     ...
     * @param array $args
     */
    function get_tag_data(array $args = [])
    {
        $data = [];
        if (count($tags = get_tags($args))) {
            foreach ($tags as $tag) {
                $data[] = [
                    'id' => $tag->id,
                    'name' => htmlentities($tag->name)
...
if (!function_exists('add_shortcode')) {
     ...
     * @return mixed
     */
    function add_shortcode($tag, $calback)
    {
        return ShortCode::getInstance()->add($tag, $calback);
    }
}
...
if (!function_exists('remove_shortcode')) {
     ...
     * @return mixed
     */
    function remove_shortcode($tag=null)
    {
        return ShortCode::getInstance()->remove($tag);
    }
}
