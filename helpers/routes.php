...
if (!function_exists('add_web_module_routes')) {
     ...
     * @return Module|null
     */
    function add_web_module_routes(
        $controller = null, $list = [], $routeName = null, $scope = '', 
        $registerModule = false, $moduleSlug = null, 
    ...
    )
    {
        ...
        if ($routeName) {
            ...
            $groupRouter->group(function ($routerg) use ($list, $routeData, $routeName, $masterModule, $rd, $registerModule, $moduleName, $moduleSlug, $parent, $scope, $group, $routeDataMap, $moduleRouter) {
                $submodules = [];
                foreach ($list as $key) {
                                ...
                                foreach ($routeDataMap[$key] as $subslug) {
                                    if (!array_key_exists($subslug, $submodules)) {
                                        $submodules[$subslug] = [
                                            ...
                                            'actions' => [
                                                $key => [
                                                    'type' => PermissionModule::TYPE_ACTION,
                                                    'name' => ucfirst($key),
                                                    'slug' => $key,
                                                    'route' => $info['name'] ?? $key,
