$(function () {
    const Component = function (options) {
        ...
        this.changeComponentSelected = function (component_id) {
            ...
            App.api.post(this.urls.inputs, data).then(function (rs) {
                if (rs.status) {
                    var inputs = rs.data || '';
                    ...
                    var tag = $('.componnent-data-input .crazy-tag');
                    if (tag.length) {
                        tag.each(function (i, el) {
                            App.tagInput.add(el);
                        });
                    }
                    var $cs = $('.componnent-data-input .input-gallery');
                    if ($cs.length) {
                        $cs.each(function (i, el) {
                            App.galleryInput.add(el);
                        });
                    }
                    var ckeditors = $('#component-form').find('textarea.text-editor, textarea.crazy-ckeditor');
                    if (ckeditors.length) {
                    ...
                    var tinyeditors = $('#component-form').find('textarea.tiny-mce');
                    if (tinyeditors.length) {
                        tinyeditors.each(function (i, e) {
                            addTinymceEditor($(e));
                        })
                    }
