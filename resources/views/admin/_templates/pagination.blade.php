...
$url = url_merge($current_url, compact('search', 'sortby', 'sorttype', 'per_page', 'page'));
            ...
            @if ($totalPage > 1)
                <ul class="pagination">
                    ...
                    @for ($i = 1; $i <= $totalPage; $i++)
                        @if (($pageNumber > $break_point && in_array($i, [1, 2])) || ($pageNumber + $break_point < $totalPage - 2 && $i > $totalPage - 2) || ($i >= $pageNumber - $break_point && $i < $pageNumber + $break_point) || ($pageNumber < $break_point && $i <= $showButton))
