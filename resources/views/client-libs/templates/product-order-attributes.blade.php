...
@if ($variant_attributes = $product->getVariantAttributes())
    <div class="{{$sec_class}} {{parse_classname('product-variants')}}">
        @foreach ($variant_attributes as $attribute)
            ...
            ?>
        <figure class="{{$att_class}} {{parse_classname('product-attribute-item','product-variant-'.$avt,'product-attribute-'.$avt, 'product-'.$attribute->name)}}">
            <div>
                ...
                @if ($avt != 'default')
                    <ul class="{{$avl_class}} {{parse_classname('product-attribute-values', 'product-attribute-'.$avt.'-values')}} {{$attribute->list_class}}">
                        ...
                        @foreach ($attribute->values as $attrValue)
                            <li class="{{$avi_class}} {{parse_classname('variant-value-item','product-attribute-value-item', 'pav-item')}} {{$attribute->use_thumbnail && $attrValue->thumbnail?'pav-has-thumbnail': ''}}" data-thumbnail="{{$attrValue->thumbnail}}" data-value-id="{{$attrValue->value_id}}">
                                <input type="radio" id="{{$inputIdPrefix}}-variants-{{$attribute->name}}-{{$attrValue->value_id}}" name="attrs[{{$attribute->name}}]" class="{{$rad_class}} {{parse_classname('radio-value-input')}} {{$attrValue->item_class}}" data-value-id="{{$attrValue->value_id}}" value="{{$attrValue->value_id}}" @if(($def && $def == $attrValue->value_id) || (!$def && $loop->index == 0)) checked @endif>
                                <label for="{{$inputIdPrefix}}-variants-{{$attribute->name}}-{{$attrValue->value_id}}" class="{{$lab_class}} " @if ($avt == "color") style="background-color:{{$attrValue->advance_value}}" @endif>
                                    @if ($avt == 'image')
                    ...
                    <ul class="{{$avl_class}} {{parse_classname('product-attribute-values')}} {{$attribute->list_class}}">
                        ...
                        @foreach ($attribute->values as $attrValue)
                            <li class="{{$avi_class}} {{parse_classname('variant-value-item','product-attribute-value-item', 'pav-item')}}">
                                <input type="radio" id="{{$inputIdPrefix}}-variants-{{$attribute->name}}-{{$attrValue->value_id}}" name="attrs[{{$attribute->name}}]" class="{{$rad_class}} {{parse_classname('radio-value-input')}} {{$attrValue->item_class}}" data-value-id="{{$attrValue->value_id}}" value="{{$attrValue->value_id}}" @if(($def && $def == $attrValue->value_id) || (!$def && $loop->index == 0)) checked @endif>
                                <label for="{{$inputIdPrefix}}-variants-{{$attribute->name}}-{{$attrValue->value_id}}" class="{{$lab_class}} ">
                                    <span>{{$attrValue->text}}</span>
                                </label>
                            </li>
                        @endforeach
    ...
    @endphp
        @foreach ($order_options as $attribute)
            <?php
                $avt = $attribute->advance_value_type;
            ?>
            <figure class="{{$att_class}} {{parse_classname('product-attribute-item', 'product-attribute-'.$avt, 'product-'.$attribute->name)}}">
                <div>
                    <figcaption class="{{$atn_class}} ">{{$attribute->label}}</figcaption>
                    @if ($avt != 'default')
                        <ul class="{{$avl_class}} {{parse_classname('product-attribute-values', 'product-attribute-'.$avt.'-values')}} {{$attribute->list_class}}">
                            ...
                            @foreach ($attribute->values as $attrValue)
                                <li class="{{$avi_class}} {{parse_classname('variant-value-item','product-attribute-value-item', 'pav-item')}}">
                                    <input type="radio" id="{{$inputIdPrefix}}-attribute-{{$attribute->name}}-{{$attrValue->value_id}}" name="attrs[{{$attribute->name}}]" class="{{$rad_class}} {{parse_classname('radio-value-input')}} {{$attrValue->item_class}}" data-value-id="{{$attrValue->value_id}}" value="{{$attrValue->value_id}}" @if(($def && $def == $attrValue->value_id) || (!$def && $loop->index == 0)) checked @endif>
                                    <label for="{{$inputIdPrefix}}-attribute-{{$attribute->name}}-{{$attrValue->value_id}}" class="{{$lab_class}} " @if ($avt == "color") style="background-color:{{$attrValue->advance_value}}" @endif>
                                        @if ($avt == 'image')
                                        <img src="{{$attrValue->advance_value}}" alt="{{$attrValue->text}}" title="{{$attrValue->text}}" class="{{$img_class}}">
                                        @endif
                                        <span>{{$attrValue->text}}</span>
